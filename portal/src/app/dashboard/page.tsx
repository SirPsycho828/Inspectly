...
import { ClipboardList, FileText, Clock, CalendarDays, MapPin, Loader2, ArrowRight, Smartphone, Download } from 'lucide-react';
...
export default function DashboardHome() {
  ...
  const statCards = [
    { label: 'Total Inspections', value: stats.total, icon: ClipboardList, color: 'bg-accent/10 text-accent', href: '/dashboard/inspections' },
    { label: 'Published Reports', value: stats.published, icon: FileText, color: 'bg-success/15 text-success', href: '/dashboard/reports' },
    { label: 'In Progress', value: stats.inProgress, icon: Clock, color: 'bg-warning/15 text-warning', href: '/dashboard/inspections' },
    { label: 'This Month', value: stats.thisMonth, icon: CalendarDays, color: 'bg-muted text-muted-foreground', href: '/dashboard/inspections' },
  ];
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {isNewUser
            ? 'Get started by downloading the Inspectly mobile app and running your first inspection.'
            : 'Here is an overview of your inspection activity.'}
        </p>
      </div>

      {/* Getting Started — only for new users with zero inspections */}
      {isNewUser && (
        <div className="space-y-3">
          <h2 className="font-heading text-lg font-600 text-foreground">Getting Started</h2>
          <GuidanceTip id="dashboard-portal-role">
            This dashboard is your web companion for viewing inspections and reports. All inspections are created from the Inspectly mobile app — download it to get started.
          </GuidanceTip>
          <NextStepCard
            title="Download the mobile app"
            description="Create inspections, capture findings, and publish reports from your phone."
            href="#"
          ...
          <NextStepCard
            ...
            description="Once the app is installed, create an inspection to start documenting findings."
      ...
      {!isNewUser && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {statCards.map((s) => {
            const Icon = s.icon;
            return (
              <Link key={s.label} href={s.href} className="card-interactive rounded-lg border border-border bg-card p-5 shadow-sm">
      ...
      <div className="rounded-lg border border-border bg-card shadow-sm">
        ...
        {recentInspections.length === 0 ? (
          <EmptyState
            ...
            description="Inspections are created from the Inspectly mobile app. Download it to run your first inspection and your activity will appear here."
            action={isNewUser ? undefined : { label: 'View Templates', href: '/dashboard/templates' }}
          />
        ) : (
          <div className="divide-y divide-border">
            {recentInspections.map((insp: any) => (
              <Link key={insp.id} href="/dashboard/inspections" className="flex items-center gap-4 px-6 py-4 transition-colors hover:bg-muted/50">
                ...
                <span className="hidden text-xs text-muted-foreground sm:block">{formatDate(insp.updatedAt)}</span>
              </Link>
            ))}
          </div>
        )}
